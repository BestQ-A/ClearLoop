# Traycer Refactoring Workflow — 逐字拆解

源路径：`external/traycer/extracted/extension/resources/default-workflows/refactoring-workflow/`

## 1. workflow.json 完整内容

```json
{
  "id": "c4e7a1b2-3d5f-6e8a-9b0c-1d2e3f4a5b6c",
  "name": "Traycer Refactoring Workflow",
  "description": "A lean, collaborative workflow for systematic code refactoring - from understanding through validated implementation.",
  "entrypointCommand": "trigger-workflow.md",
  "commands": [
    "referred/plan-refactor.md",
    "referred/architecture-validation.md",
    "referred/ticket-breakdown.md",
    "referred/execute.md",
    "referred/verification.md"
  ]
}
```

**结构说明**：与 plan-workflow 同样的模式——只有 entrypoint + commands 文件清单，DAG 编排在每个 `.md` 的 frontmatter 内。注意 entrypoint 文件名是 `trigger-workflow.md`（连字符），不同于其他两个 workflow 的 `trigger_workflow.md`（下划线）。

## 2. step 列表表格

| step id | name | prompt 文件 | selectedAgent | 入参 hint | nextSteps | 输出 artifact |
|---|---|---|---|---|---|---|
| 0 | trigger / Traycer Refactoring Workflow | `trigger-workflow.md` | (默认) | Code area to refactor, the problem you're facing, or the refactoring goal | plan-refactor | (readonly 需求验证) |
| 1 | plan-refactor | `referred/plan-refactor.md` | (默认) | Specific areas of concern, constraints, or priorities | architecture-validation, ticket-breakdown | `refactoring-analysis.md` + `refactoring-approach.md` |
| 2 | architecture-validation | `referred/architecture-validation.md` | REVIEWER | Specific invariants, migration risks, or hotspots to focus validation on | ticket-breakdown | (原地 update analysis & approach) |
| 3 | ticket-breakdown | `referred/ticket-breakdown.md` | (默认) | Specific constraints on ticket size or sequencing | execute, verification | ticket 列表 + mermaid 依赖图 |
| 4 | execute | `referred/execute.md` | (默认) | Specific tickets to execute, or "all" for batch execution | verification | code diff + ticket 状态 |
| 5 | verification | `referred/verification.md` | REVIEWER | Specific areas to focus verification on, or concerns that arose during implementation | execute, plan-refactor | 三路决策（Approve / Fix Tickets / Re-plan） |

**特色 step**：

- `plan-refactor` 是 plan-workflow 中 plan step 的"refactor 专属版"——拆 Part 1 (Analysis) + Part 2 (Approach) 两阶段，强调 invariants（行为/契约/性能/数据 4 类不可变量）
- `verification` 是三路分支决策（plan workflow 的 implementation-validation 没有这种显式三路）

## 3. trigger-workflow.md 全文
...
description: A lean, collaborative workflow for systematic code refactoring - from understanding through validated implementation
argumentHints:
  - Code area to refactor, the problem you're facing, or the refactoring goal
nextSteps:
  - name: "plan-refactor"
---
...
### 4.1 `referred/plan-refactor.md`

```markdown
---
description: Analyze the refactoring impact and collaboratively define the technical approach through structured decision-making.
argumentHints:
  - Specific areas of concern, constraints, or priorities
nextSteps:
...
### 4.2 `referred/architecture-validation.md`

```markdown
---
description: Stress-test the refactoring approach for safety, simplicity, and codebase fit before breaking it into tickets.
argumentHints:
  - Specific invariants, migration risks, or hotspots to focus validation on
...
```markdown
---
description: Break down the refactoring approach into concrete, sequenced implementation tickets.
argumentHints:
  - Specific constraints on ticket size or sequencing
nextSteps:
  - name: "execute"
  - name: "verification"
---
...
### 4.5 `referred/verification.md`

```markdown
---
description: Review implementation against the plan, identify issues, and either approve or create corrective action.
argumentHints:
  - Specific areas to focus verification on, or concerns that arose during implementation
selectedAgent: REVIEWER
nextSteps:
  - name: "execute"
  - name: "plan-refactor"
