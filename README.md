...
1. **Install Extension**:
   ...
   - Search "ClearLoop" in VS Code Extensions view (`Ctrl+Shift+X`) after a package is published.
...
4. **Usage**:
   ...
   - Use `ClearLoop: Create CLI Agent Handoff Smoke`.
   - Inspect the generated `.bestqa/agent-runs/<run>/` directory.
   - Run the chosen local agent explicitly, then record evidence and verification.
